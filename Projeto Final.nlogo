...
to grow-grass  ; patch procedure
  ...
  if (pcolor = brown) and (river-neighbors > 1) [
    ifelse countdown <= 0
      [ set pcolor green
        set countdown grass-regrowth-time ]
...
to do-rain
  ;if pcolor != blue[
    ifelse countdown-rain <= 0 [
      set countdown-rain 360
      set bool-rain random 1000
        if (bool-rain < water-perc) and (pcolor = brown) [
          set pcolor blue]]
...
to do-river
  ask patches [ set dirt-neighbors count neighbors with [pcolor = brown] ]
  ask patches [ set river-neighbors count patches in-radius 5 with [pcolor = blue] ]

  ask patches [ if dirt-neighbors >= 6
      [change-color-river]]
end
