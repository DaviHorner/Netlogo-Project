...
patches-own [
  ...
  dirt-neighbors
  river-neighbors
]    ; this is for the sheep-wolves-grass-river model version
...
to go
  ...
  ask patches [
    grow-grass ; method to grow grass
    do-rain    ; method to implemment rain
  ]
...
to display-labels  ; display the energy of the turtles
...
to grow-grass  ; patch procedure
  ...
  if (pcolor = brown) and (river-neighbors > 1) [  ; it requires the patch to be brown and be near a 'river'
    ifelse countdown <= 0                          ; countdown on brown patches: if you reach 0, grow some grass
      [ set pcolor green
        set countdown grass-regrowth-time ]
...
to do-rain
    ...
    ifelse countdown-rain <= 0 [                               ; countdown to rain if reach 0, it rains
      set countdown-rain 360                                   ; reset countdown to 360
      set bool-rain random 1000                                ; create a random number to determine if it will rain on that patch
        if (bool-rain < water-perc) and (pcolor = brown) [     ; if the patch is brown and the number generated is smaller than the water-perc it will turn that patch blue
          set pcolor blue]]
     [ set countdown-rain countdown-rain - 1 ]

end

to do-river
  ask patches [ set dirt-neighbors count neighbors with [pcolor = brown] ]               ; count the number of neighbors that is dirt
  ask patches [ set river-neighbors count patches in-radius 5 with [pcolor = blue] ]     ; count the number of neighbors that is blue on a radius

  ask patches [ if dirt-neighbors >= 6                                       ; swap river to dirt if there are too much dirt around a river
      [change-color-river]]
end
...
"river" 1.0 0 -13345367 true "" "plot count river / 4"
...
The second variation,  the "sheep-wolves-grass-water" version add the patch of water(blue) to the simulation, and create "rain" where after 360 ticks the simulation generate a certain amount of blue patches where before where brown patches.

The construction of this model utilized the predation model from Netlogo 6.2

## HOW TO USE IT

1. Set the bool-water? to ON to choose "sheep-wolves-grass-water" model and to include water, or to OFF to choose "sheep-wolves-grass" to only include wolves (black) and sheep (white) and grass.
...
Parameters:
BOOL-WATER?: Whether we model sheep, wolves, grass and water or just sheep, wolves and grass
...
WATER-PERC: To set the percentage of water created on an scale of 1000
...
Create a new type of patch for degraded dirt(sand/rocks) where it makes more difficult to grow grass and sustain water. 

Can you modify the model so the sheep will flock and keep near grass?

Can you modify the model so the wolves will flock and keep away from sheeps until time to hunt?

Can you modify the model so that wolves actively chase sheep?

Can you modify the model so that sheep will actively flee from wolves?

Can you modify the model so to make the rain random?

## NETLOGO FEATURES

Note the use of breeds to model two different kinds of "turtles": wolves and sheep. Note the use of patches to model grass and water.
